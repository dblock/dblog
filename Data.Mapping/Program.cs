...
namespace DBlog.Data.Mapping
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                Console.WriteLine("DBlog.Data.Mapping: fix hibernate mappings");

                if (args.Length == 0)
                {
                    Console.WriteLine("syntax: DBlog.Data.Mapping.exe [Project.csproj]");
                ...
                foreach (AdditionalProjectFileConfigurationElement interfaceFile in additionalProjectFiles.AdditionalProjectFiles)
                {
                    ...
                    if (compileIncludeNode == null)
                    {
                        ...
                        XmlElement subtypeElement = null;
                        switch (interfaceFile.SubType)
                        {
                            case AdditionalProjectFileConfigurationSubType.File:
                                subtypeElement = projectXml.CreateElement("SubType", msbuildns);
                                subtypeElement.AppendChild(projectXml.CreateTextNode("Code"));
                                break;
                            case AdditionalProjectFileConfigurationSubType.Link:
                                subtypeElement = projectXml.CreateElement("Link", msbuildns);
                                subtypeElement.AppendChild(projectXml.CreateTextNode(interfaceFile.Filename));
                                break;
                        }
                ...
                foreach (string mappingFileName in Directory.GetFiles(projectDirectory, "*.hbm.xml"))
                {
                    ...
                    if (hibernateMappingNode != null)
                    {
                        ...
                        {
                            XmlNodeList propertyNodes = hibernateMappingNode.SelectNodes(string.Format(
                                "//hns:property[@name='{0}'][@type='String']", clobProperty.Property), mappingXmlNsMgr);
                            foreach(XmlNode propertyNode in propertyNodes)
                            {
                                Console.WriteLine("  Property: {0}: {1}", 
                                    propertyNode.ParentNode.Attributes["name"].Value,
