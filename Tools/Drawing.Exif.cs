...
namespace DBlog.Tools.Drawing.Exif
{
    ...
    public class EXIFPropertyItem
    {
        ...
        public override string ToString()
        {
            switch (EXIFType)
            {
                case EXIFPropertyTypes.ExifTypeString:
                    return ParsedString;
                case EXIFPropertyTypes.ExifTypeUnsignedRational:
                case EXIFPropertyTypes.ExifTypeSignedRational:
                    return ParsedRational.ToString();
                case EXIFPropertyTypes.ExifTypeSignedInt:
                case EXIFPropertyTypes.ExifTypeUnsignedInt:
                case EXIFPropertyTypes.ExifTypeSignedChar:
                case EXIFPropertyTypes.ExifTypeUnsignedChar:
                    return ParsedInt.ToString();
                default:
                    return EXIFType.ToString() + " (" + id.ToString() + "/" + type.ToString() + ") " + Value.Length.ToString();
            }
        }
        ...
        public EXIFInt ParsedInt
        {
            get
            {
                EXIFInt parsed = new EXIFInt(this.data);
                return parsed;
            }
        }
    ...
    public class EXIFMetaData
    {
        ...
        public EXIFPropertyItem[] EXIFPropertyItems
        {
            get
            {
                return mEXIFPropertyItems;
            }
            set
            {
                mEXIFPropertyItems = value;
            }
        }
        ...
        public string ToString(bool fIncludeUnknownCodes, bool fIncludeUndefinedValues)
        {
            StringBuilder Result = new StringBuilder();
            for (int i = 0; i < mEXIFPropertyItems.Length; i++)
            {
                if ((fIncludeUnknownCodes || mEXIFPropertyItems[i].IsKnownCode)
                 && (fIncludeUndefinedValues || mEXIFPropertyItems[i].EXIFType != EXIFPropertyTypes.ExifTypeUndefined))
                {
                    Result.Append(mEXIFPropertyItems[i].EXIFCode.ToString());
                    Result.Append(": ");
                    Result.Append(mEXIFPropertyItems[i].ToString());
                    Result.Append("\n");
                }
            }
